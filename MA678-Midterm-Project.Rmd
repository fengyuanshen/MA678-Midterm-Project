...
split <- sample.split(listings_data$price, SplitRatio = 0.8)

# Split data set
train_data <- listings_data[split, ]
test_data <- listings_data[!split, ]
```
...
```{r echo=FALSE}
# Residual plot
plot(fitted(fit_null), resid(fit_null), col = "#003566",
...
#### Model2: Complete pooling (Linear Regression)
...
plot(fitted(fit_lin), resid(fit_lin), col = "#003566",
     main = "Residual Plot of Linear Model", xlab = "Fitted Values", ylab = "Residuals")
abline(h = 0, col = "#D62828") # Add a horizontal line at y=0
```

```{r echo=FALSE}
# Q-Q Plot
qqnorm(resid(fit_lin), col = "#003566", main = "Q-Q Plot of Linear Model")
...
ggplot(coef_df, aes(x = term, y = Estimate)) +
  ...
  labs(
    title = 'Coefficient Estimates of Linear Model',
    x = '', y = 'Estimate'
    )
```
...
#### Model3: Linear Regression with Interaction Term
...
plot(fitted(fit_lin2), resid(fit_lin2), col = "#003566",
     main = "Residual Plot of Linear Model with Interaction Term",
...
```{r echo=FALSE}
# Q-Q Plot
qqnorm(resid(fit_lin2), col = "#003566", main = "Q-Q Plot of Linear Model with Interaction Term")
...
ggplot(coef_df2, aes(x = term, y = Estimate)) +
  ...
  labs(
    title = 'Coefficient Estimates of Linear Model with Interaction Term',
    x = '', y = 'Estimate'
    )
```
...
```{r echo=FALSE}
fit_partial_pooling <- lmer(price ~ host_is_superhost + latitude + longitude + room_type + 
                accommodates + bathrooms + bedrooms + beds + minimum_nights +
                number_of_reviews + review_scores_rating + review_scores_location +
                latitude:longitude + accommodates:bedrooms +
                review_scores_rating:review_scores_location + factor(host_neighbourhood) +
                (1 + | host_neighbourhood), data = train_data)
fit_partial_pooling
```
...
RMSE_per_point <- sqrt((log(test_data$price) - test_predictions1)^2)
...
# RMSE
rmse2 <- sqrt(mean((log(test_data$price) - test_predictions2)^2))
```

```{r}
# Calculate RMSE of each data point
RMSE_per_point <- sqrt((log(test_data$price) - test_predictions2)^2)
...
ggplot(test_data, aes(x = price, y = exp(test_predictions2), color = RMSE_per_point)) +
  ...
  scale_color_gradientn(colours = c("#D00000", "#DC2F02", "#F9844A", "#F9C74F", "#90BE6D",
                                    ...
                                    "#023E8A", "#013A63", "#012A4A", "#03045E")) +
  labs(
    title = "True Values vs Predictions - Complete Pooling (Linear Regression)",
    x = "True Value", y = "Predictions"
    ) +
  geom_abline(intercept = 0, slope = 1, color = "black", size = 0.8) +
  ylim(0, 600) +
...
# RMSE
rmse3 <- sqrt(mean((log(test_data$price) - test_predictions3)^2))
```

```{r}
# Calculate RMSE of each data point
RMSE_per_point <- sqrt((log(test_data$price) - test_predictions3)^2)
...
ggplot(test_data, aes(x = price, y = exp(test_predictions3), color = RMSE_per_point)) +
  ...
  scale_color_gradientn(colours = c("#D00000", "#DC2F02", "#F9844A", "#F9C74F", "#90BE6D",
                                    ...
                                    "#023E8A", "#013A63", "#012A4A", "#03045E")) +
  labs(
    title = "True Values vs Predictions - Linear Regression with Interaction Term",
    x = "True Value", y = "Predictions"
    ) +
  geom_abline(intercept = 0, slope = 1, color = "black", size = 0.8) +
  ylim(0, 600) +
