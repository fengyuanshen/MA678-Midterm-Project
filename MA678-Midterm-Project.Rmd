...
```{r}
fit_null <- lm(price ~ 1, data = listings_data)
summary(fit_null)
```
...
```{r echo=FALSE}
fit_lin <- lm(log(price) ~ host_is_superhost + latitude + longitude + room + accommodates + 
             bathrooms + bedrooms + beds + minimum_nights + number_of_reviews +
             review_scores_rating + review_scores_location, data = listings_data)
summary(fit_lin)
```
...
```{r}
# Get fixed effect estimates directly 
coef_fit_lin <- coef(fit_lin)

# Create data frames for coefficient estimates 
coefest_fit_lin <- data.frame(
  Est = coef_fit_lin,
  Se = se(coef_fit_lin),
  names = names(coef_fit_lin)
  )

# Graphical representation for fit4 
ggplot(coefest_fit_lin, aes(x = names, y = Est)) +
  geom_errorbar(aes(ymin = Est - 2 * Mad, ymax = Est + 2 * Mad), width = 0) + 
  geom_point() +
  labs(title = "Coefficient Estimates for fit4", x = "Coefficient", y = "Estimate") +
  theme_minimal() + 
  theme( 
    plot.title = element_text(hjust = 0.5) 
    )
...
ggplot(coef_df, aes(x = term, y = Estimate)) +
  ...
  labs(x = 'Terms', y = 'Estimate', title = 'Coefficient Estimates from Linear Model')
```

模型的**`Multiple R-squared`**值为0.4524，表明模型解释了大约45.24%的**`price`**变量的变异性。虽然这个值表明模型有一定的解释力，但仍有超过一半的变异性未被模型解释。

```{r}
fit_lin2 <- lm(price ~ latitude + longitude + factor(room_type) + accommodates + 
             bedrooms + beds + minimum_nights + number_of_reviews +
             review_scores_rating + latitude:longitude + accommodates:bedrooms, data = listings_data)
summary(fit_lin2)
```
...
#### Model3: No Pooling Model

no pooling就是加一个factor

#### Model4: Complete Pooling Model
...
fit_partial_pooling <- lmer(price ~ host_is_superhost + latitude + longitude + room_type + 
                            ...
                            (1 | property_type), data = listings_data)
summary(fit_partial_pooling)
```

#### Residual plot

ppcheck-train/test set
